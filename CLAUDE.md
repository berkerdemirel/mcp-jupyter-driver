...
- **`exec_lock` for any tool that writes the notebook.** Cell mutations (`add_cell`, `edit_cell`, …) and `run_cell` take `session.exec_lock` so concurrent tool calls don't interleave partial writes.
- **Read fresh on entry.** Every tool re-reads the notebook from the server (see `session.read_notebook`) — we deliberately don't cache, because the user's editor might have edited it.
